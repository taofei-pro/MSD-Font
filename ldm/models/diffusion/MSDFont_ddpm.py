...
class MSDFont_train_stage1_rec_model(LatentDiffusion):

    def __init__(self, style_stage_config, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = DiffusionWrapper_MSDFont_train_stage1_rec_model(kwargs['unet_config'], kwargs['conditioning_key'])
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        keys = list(sd.keys())
        # #################################################
        # # ignore attn2 parameters, load pretrained diffuser model
        # ignore_keys = []
        # for k in keys:
        #     if 'attn2' in k:
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        return (
                x_t - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * noise
        )
        ...
        if plot_diffusion_rows:
            ...
            for t in range(self.num_timesteps):
                if t % self.log_every_t == 0 or t == self.num_timesteps - 1:
                    ...
                    z_noisy = self.q_sample(x_start=z_start, c=c, t=t, noise=noise)
...
class DiffusionWrapper_MSDFont_train_stage1_rec_model(pl.LightningModule):
...
class MSDFont_train_stage1_trans_model_Gencase(LatentDiffusion):
    def __init__(self, style_stage_config, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class DiffusionWrapper_MSDFont_train_stage1_trans_model_Gencase(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.sequential_cross_attn = diff_model_config.pop("sequential_crossattn", False)
        self.diffusion_model = instantiate_from_config(diff_model_config)
