...
def generate_with_model(font_path, chars_info_path, output_dir, rec_model_path, trans_model_path, batch_size=16):
    ...
    try:
        ...
        # 创建条件阶段配置
        cond_stage_config = "__is_first_stage__"  # 根据配置文件设置
        ...
        try:
            ...
            device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
            try:
                checkpoint = torch.load(trans_model_path, map_location=device)
                if "state_dict" in checkpoint:
                    trans_model.load_state_dict(checkpoint["state_dict"], strict=False)
                else:
                    trans_model.load_state_dict(checkpoint, strict=False)
            except Exception as e:
                print(f"加载权重时出错，尝试不使用weights_only参数: {e}")
                checkpoint = torch.load(trans_model_path, map_location=device)
                if "state_dict" in checkpoint:
                    trans_model.load_state_dict(checkpoint["state_dict"], strict=False)
                else:
            ...
            print(f"加载第一阶段转换模型时出错: {e}")
            import traceback
            traceback.print_exc()
            return None, None
        
        # 创建 trans_model_config
        ...
        try:
            ...
            device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
            try:
                checkpoint = torch.load(rec_model_path, map_location=device)
                if "state_dict" in checkpoint:
                    rec_model.load_state_dict(checkpoint["state_dict"], strict=False)
                else:
                    rec_model.load_state_dict(checkpoint, strict=False)
            except Exception as e:
                print(f"加载权重时出错，尝试不使用weights_only参数: {e}")
                checkpoint = torch.load(rec_model_path, map_location=device)
                if "state_dict" in checkpoint:
        ...
        for j, char in enumerate(batch_chars):
            try:
                ...
                with torch.no_grad():
                    ...
                    # 使用重建模型
                    # 第一个参数是输入图像，第二个参数是条件
                    rec_output = rec_model(ref_img, c)
                    
                    # 使用转换模型
                    # 第一个参数是重建模型的输出，第二个参数是条件
                    trans_output = trans_model(rec_output, c)
                    
                    # 获取生成的图像
                    generated_img = trans_output
...
def main():
    parser = argparse.ArgumentParser(description="使用训练好的模型生成字体")
