...
def generate_font(source_font_path, target_chars, output_dir, rec_model, trans_model, batch_size=16):
    """生成字体"""
    ...
    for i in tqdm(range(0, len(chars_to_generate), batch_size)):
        ...
        # 随机选择源字体中的参考字符
        ref_chars = np.random.choice(source_chars, min(len(batch_chars), len(source_chars)), replace=True)
...
def main():
    ...
    parser.add_argument("--config_rec", type=str, default="/home/zihun/workspace/MSD-Font/configs/MSDFont/MSDFont_Eval_rec_model_predx0_miniUnet.yaml", help="重建模型配置文件")
    ...
    parser.add_argument("--config_trans", type=str, default="/home/zihun/workspace/MSD-Font/configs/MSDFont/MSDFont_Eval_trans_model_predx0_miniUnet.yaml", help="转换模型配置文件")
    ...
    rec_model, trans_model = load_models(
        args.config_rec, args.rec_model,
        args.config_trans, args.trans_model
    )
